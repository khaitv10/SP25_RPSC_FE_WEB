    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-60 backdrop-blur-sm transition-opacity duration-300">
      ...
      >
        ...
        <div className="relative mb-8">
          <h2 className="text-3xl font-bold text-center text-blue-800">Landlord Details</h2>
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-3">
              ...
              {landlord.gender && (
                <div className="flex items-center text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                  ...
                  <span className="text-sm">{landlord.gender}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {shouldShowField(landlord.companyName) && (
            <DetailCard icon={<FaBuilding className="text-blue-500" />} 
                      title="Company Name" 
                      value={landlord.companyName} />
          )}
          
          {shouldShowField(landlord.licenseNumber) && (
            <DetailCard icon={<FaIdCard className="text-orange-500" />} 
                      title="License Number" 
                      value={landlord.licenseNumber} />
          )}
          
          {shouldShowField(landlord.status) && (
            <DetailCard icon={<FaCheckCircle className="text-green-500" />} 
                      title="Status" 
                      value={landlord.status} />
          )}
          
          {shouldShowField(landlord.address) && (
            <DetailCard icon={<FaMapMarkerAlt className="text-red-500" />} 
                      title="Address" 
                      value={landlord.address} />
          )}

          {shouldShowField(landlord.createdDate) && (
            <DetailCard icon={<FaCalendarAlt className="text-purple-500" />} 
                      title="Created Date" 
                      value={formatDate(landlord.createdDate)} />
          )}
        </div>
